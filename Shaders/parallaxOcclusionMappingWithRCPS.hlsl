...
void main(in PS_IN In, out float4 outDiffuse : SV_Target)
{
	...
	{
		currHeight = g_TextureHeight.SampleGrad(g_SamplerState, In.TexCoord + currTexOffset, dx, dy).r;
    ...
    //if (gDisplayShadows && dot(lightDirWS, normalWS) < 0)
    {
        ...
        prevRayZ = g_TextureHeight.Sample(g_SamplerState, parallaxTex).r;
        prevHeight = prevRayZ;
        ...
        while (sampleIndex < sampleCount + 1)
        {
            currHeight = g_TextureHeight.SampleGrad(g_SamplerState, parallaxTex + currTexOffset, dx, dy).r;
        ...
        if (checkShadow)
        {
            shadowFactor = 1 - saturate((1 - (g_TextureHeight.Sample(g_SamplerState, parallaxTex + finalTexOffset).r - g_TextureHeight.Sample(g_SamplerState, parallaxTex).r)) * Parameter.y);
        }
    }
