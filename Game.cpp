...
void CGame::UpdateSceneImGui(const GameTimer& GlobalTimer)
{
	...
	if (showClose)
	{
		...
		if (ImGui::TreeNode("GaussBlur"))
		{
			...
			if (ImGui::DragInt(u8"MaxBlurRadius", &m_GaussBlurCB.MaxBlurRadius, 0.1f, 1, 20)) // 上限値はShader側の定数と同じように設定する
			{
		...
		if (ImGui::TreeNode("Bloom"))
		{
			...
			if (ImGui::DragInt(u8"MaxBlurRadius", &m_BloomCB.MaxBlurRadius, 0.1f, 1, 20)) // 上限値はShader側の定数と同じように設定する
			{
